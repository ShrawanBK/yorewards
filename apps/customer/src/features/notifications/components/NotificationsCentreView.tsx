...
export function NotificationsCentreView() {
  ...
  const { data: unreadCount = 0, invalidate } = useCustomerUnreadCount();
  const { data: notifications = [], isFetching } = useCustomerNotifications();
  ...
  async function handleMarkRead(id: string) {
    setError(null);
    const result = await markCustomerNotificationReadAction(id);
  ...
  async function handleMarkAllRead() {
    setError(null);
    const result = await markAllCustomerNotificationsReadAction();
    if (isActionFailure(result)) {
      setError(resolveActionError(tErrors, result.error));
      return;
    }
  ...
  return (
    <div className="mx-auto w-full max-w-lg space-y-6 px-4 py-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{t("centre.description")}</p>
      </header>
      ...
      >
        <TabsList className="flex h-auto flex-wrap gap-1">
          ...
          {CUSTOMER_NOTIFICATION_CATEGORIES.map((key) => (
