...
function DetailRow({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="flex flex-col gap-0.5 sm:flex-row sm:justify-between sm:gap-4">
      <dt className="text-sm merchant-body-muted">{label}</dt>
  ...
  return (
    <Card className="merchant-glass-card">
      ...
      <CardContent className="space-y-4">
        ...
        <p className="rounded-lg border border-border bg-muted/40 p-3 text-sm merchant-body-muted">
        ...
        {merchant.status === "rejected" && merchant.rejection_reason ? (
          <div className="rounded-lg border border-destructive/50 bg-destructive/15 p-3 text-sm">
            ...
            <p className="mt-1 merchant-body-muted">
